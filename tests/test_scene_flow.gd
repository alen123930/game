extends Node
## WS-5 场景流转测试（合并后结构）：验证 GameMain 状态机加载主菜单/城镇/探索/战斗/结算场景并互相切换。
...
func _ready() -> void:
	...
	_check(_current_is(main, "MainMenu"), "启动后进入主菜单场景")

	# 进入城镇
	main.change_state(GameMain.GameState.TOWN)
	await _wait_frames(5)
	_check(_current_is(main, "Town"), "进入城镇场景")
	...
	var dungeon_node: Node = main.get("_current_scene")
	...
	# 撤退 → 结算
	var dungeon2: Node = main.get("_current_scene")
	...
	# 返回城镇
	var result_node: Node = main.get("_current_scene")
...
func _current_is(main: Node, node_name: String) -> bool:
	var cs: Node = main.get("_current_scene")
