...
func _format_event(entry: Dictionary) -> String:
	...
	match t:
		...
		"mental_resolve":
			return "R%d %s 精神判定：%s（%s）" % [round, _un(entry.get("unit", -1)), ("美德" if entry.get("resolution", "") == "virtue" else "受难"), entry.get("crisis", "")]
		"crisis_action":
			...
			if skill_name != "":
				...
				return "R%d %s[受难·%s] 行动：%s → %s" % [round, _un(entry.get("unit", -1)), entry.get("crisis", ""), skill_name, _un(int(entry.get("target_uid", -1)))]
			return "R%d %s[受难·%s] 空放（不行动）" % [round, _un(entry.get("unit", -1)), entry.get("crisis", "")]
		"stress_death":
			return "R%d %s 压力崩溃，立即死亡！" % [round, _un(entry.get("unit", -1))]
		"heal_blocked":
			return "R%d %s 无法被治疗（%s）" % [round, _un(entry.get("target", -1)), entry.get("crisis", "")]
		"heal":
