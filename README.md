...
- **遗迹地图 + 探索循环（WS-5，WS-19 对齐暗黑地牢）**：程序化网格地图生成（4×4~6×5，战斗/奇物/宝箱/目标/安全/起始/关底房 + 走廊节点），走廊可含陷阱（可拆除或触发）或障碍（碎石/藤蔓需铲子清除）；**进入新房间自动侦查掷骰**（受火把档位/技能/怪癖/饰品修正，成功揭示地图，移除「侦查→探索→检查」三步流程）；**任务类型对齐 4 类**（探索=到达目标房 / 收集=收集指定物 / 狩猎与 Boss=击杀关底 Boss）；任务长度露营接口（中=1、长=2，营地系统后续独立任务）；**撤退无惩罚**（随时撤退保留全部战利品，移除 70% 结算折扣）、陷阱（铲子/徒手拆除/直接触发）与门锁（钥匙/铲子/盗贼撬锁）、团队火把（0~100，三档效果曲线）、遇敌切战斗、结算回城。
...
> 待办：WS-10 掉落与经济；WS-11 存档。WS-6 美术 V0.1 / WS-12 美术 V0.2 素材已入库（`assets/art/`，供 V0.2 城镇经营等任务按 GDD 6.3 引用）。
...
tests/               无头自检（WS-3 冒烟 + WS-5/WS-19 生成器/侦查/走廊/全流程/场景流转 + WS-4 战斗 + WS-7 压力火把 + WS-8 战斗触屏 UI + WS-9 城镇经营 + WS-14 怪癖/伤病/疾病 + WS-15 剧情）
...
# WS-5 生成器 + 火把档位 + 全流程 + WS-19 侦查/任务类型/撤退（以 test_main.tscn 为主场景）
godot --headless --path . res://tests/test_main.tscn

# WS-19 探索对齐：走廊障碍/陷阱/任务完成（场景层）
godot --headless --path . res://tests/test_ws19.tscn
