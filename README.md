...
# WS-11 存档系统（全量捕获/恢复/手动3槽/自动存档/完整性校验降级 + 场景集成）
godot --headless --path . res://tests/test_ws11.tscn
...
- 全量 JSON 存档：`user://saves/slot_<n>.json`（手动槽 1~3，覆盖城镇状态 + 英雄 + 任务进度，版本号与完整性校验，损坏/版本不符/缺字段降级为空档不崩溃）。
- 自动存档：`user://saves/autosave.json`（每探索节点、每战斗后写入；主菜单「继续」优先读取）。
